...
({1:[function(require,module,exports){
    ...
    */
    ...
    var mergehull = require('./point-location'); 
    ...
    var clear = function() { 
        drawTriangle(mergehull.outer_triangle, "black", "white"); 
    };
    ...
    var reset = function() { 
        ...
        var handleClick = function() {
            ...
            if (util.triangleContainsPoint(p, mergehull.outer_triangle)) {
                if (!_.isEmpty(points)) { 
    ...
    buttons.findIndependentSet.on("click", function() { 
        set = mergehull.findIndependentSet(graph); 
    ...
    buttons.remove.on("click", function() { 
        var tris = mergehull.removeVertices(graph, set, dag); 
        clear(); 
        drawGraph(graph, "black"); 
    ...
    },{"./point-location":3,"./util":4,"underscore":6}],
    ...
    */
    ...
    Vertex.prototype.addSquare = function(square_id){
        if (!_.contains(this.squares, square_id)) { 
            this.squares.push(square_id); 
    }
    }
    ...
    var getSquarelation = function(graph, square, hole){
        var output = []
        output.push(square);
        return output;
    } 
    ...
    var squarelate = function(graph, square, polygon){
        var squ = getSquarelation (graph, sqaure, hole);
        _.each(squ, function(t) {
          graph.connect(t[0], t[1]);
          graph.connect(t[1], t[2]); 
          graph.connect(t[2], t[3]);
          graph.connect(t[3]. t[0]); 

          // var s = new Square(t[0], t[1], t[2], t[3]);
          // var square_id = 
        });
    }
    ...
    var run = function(pts) { 
        ...
        graph.addVertex(outer_boundary[0].x, outer_boundary[0].y);
        graph.addVertex(outer_boundary[1].x, outer_boundary[1].y);
        graph.addVertex(outer_boundary[2].x, outer_boundary[2].y);
        graph.addVertex(outer_boundary[3].x, outer_boundary[3].y);

    
        graph.connect(pts.length, pts.length+1); 
        graph.connect(pts.length+1, pts.length+2); 
        graph.connect(pts.length+2, pts.length+3)
        graph.connect(pts.length, pts.length+3); 
    
        var outer = [pts.length, pts.length+1, pts.length+2, pts.length + 3]; 
        var outer_squares = squarelate(graph, outer, polygon); 
        console.log("test");
        graph.triangulations.push(_.union(inner_triangles, outer_squares)); 
        // graph.triangulations.push(outer_triangles);
        // graph.triangulations.push(inner_triangles)
    ...
    },{"./dag":1,"./util":4,"earcut":5,"underscore":6}],
